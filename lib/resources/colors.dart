
import 'package:flutter/material.dart';

class AppColor{
  Color primaryText = const Color(0xFF323C5F);

  Color primaryDescription = const Color(0xFFA8ACB4);
  Color primaryGreyText = const Color(0xFF797B89);

  Color primaryGreyBox=const Color(0xFFfefefe);
  Color pendingDeal=const Color(0xFFDAFFCE);
  Color green=const Color(0xFF4EE03C);
  // buttons
Color primaryButtonText = const Color(0xFFFFDE33);
Color tertiaryButtonText= const Color(0xFF3865FF);
}